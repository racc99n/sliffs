...
async function handleCheckBalance(userId, replyToken, linkData) {
    try {
        ...
        await replyMessage(replyToken, {
            type: 'text',
            text: `💰 ข้อมูลบัญชี\n\n💰 ยอดเงิน: ${(data.balance || 0).toLocaleString()} บาท\n💎 คะแนนสะสม: ${(data.points || 0).toLocaleString()} คะแนน\n🏆 ระดับสมาชิก: ${data.tier || 'Bronze'}`
        });
...
async function handleCheckPoints(userId, replyToken, linkData) {
    try {
        ...
        await replyMessage(replyToken, {
            type: 'text',
            text: `💎 คะแนนสะสม: ${(data.points || 0).toLocaleString()} คะแนน\n🏆 ระดับสมาชิก: ${data.tier}`
        });
...
async function handleTransactionHistory(userId, replyToken, linkData) {
    try {
        ...
        transactions.rows.forEach((tx, index) => {
            ...
            message += `${index + 1}. ${emoji} ${tx.transaction_type === 'deposit' ? 'ฝาก' : 'ถอน'}: ${(tx.amount || 0).toLocaleString()} บาท\n`;
            message += `   วันที่: ${date}\n\n`;
        });

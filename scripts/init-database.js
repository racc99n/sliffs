...
async function createInlineSchema() {
    return `
-- Prima789 LINE Member Card Database Schema - Inline
    ...
    for (const tableName of expectedTables) {
        try {
            ...
            if (result.rows[0].count > 0) {
                ...
                console.log(`✅ Table '${tableName}' exists with ${cols.rows[0].count} columns`);
            } else {
...
async function runDatabaseTests() {
    try {
        ...
        await pool.query('DELETE FROM transactions WHERE transaction_id = $1', ['test_init_txn']);
        await pool.query('DELETE FROM account_links WHERE line_user_id = $1', ['test_user_init']);
        await pool.query('DELETE FROM prima789_accounts WHERE username = $1', ['test_init_user']);
        await pool.query('DELETE FROM line_users WHERE line_user_id = $1', ['test_user_init']);
    ...
    try {
        ...
        for (const account of sampleAccounts) {
            await pool.query(`
                ...
                ON CONFLICT (username) DO NOTHING
            `, [
                account.username, account.mm_user, account.first_name, account.last_name,
                account.tel, account.bank_name, account.available, account.credit_limit,
                account.tier, account.points
            ]);
...
module.exports = { initializeDatabase, verifyTables, checkExistingData, runDatabaseTests };